...
***NOTE: (that is, we want to find the earliest moment when all the positions from 1 to X are covered by leaves). 
...
function frogRiverOne(X, A) {
  ...
  for (let i = 0; i < A.length; i++) {
    ...
    if (A[i] <= X && !visited[leafPos]) {
      visited[leafPos] = true;
      frogPos += 1;

      if (frogPos === X) return i;
    }
  }
